...
SummarizeInlaVars = function(x, nSamples = 1000) {
...
SummarizeInlaVars(fitWCF)

summarise_precision_to_variance <- function (y) {
  inla.tmarginal(function(x) 1/x,
                 fitWCF$marginals.hyperpar$`Precision for y`) %>%
    inla.zmarginal()
}
summarise(herdI)
...
MeshB <- inla.mesh.2d(Locations, max.edge = c(20, 40)) # Error in plot.new() : figure margins too large
