...
length(unique(data_clean$COW_ID))
#=======================================================================
...
### Filtering herd with at least Median records(24) to test the model
 ...
 data_clean_final <- data_clean_test_3 
 data_clean_final$mean <- rep(1, nrow(data_clean_test_3))
 
 ## Save data clean_test_3 and data_clean_final as .csv
 write.csv(data_clean_test_3, "/Users/ihouaga/Documents/Project1/data_clean_test_3.csv")
 write.csv(data_clean_final, "/Users/ihouaga/Documents/Project1/data_clean_final.csv")
...
# Export final data_clean_final for blupf90
 summary(data_clean_final$region)
sum(is.na(data_clean_final$region))# 0 missing data for region
sum(is.na(data_clean_final)) #0 missing data in the dataset

#Export_data_clean 
write.table(data_clean_final, "data_clean_final.dat",
            quote = FALSE, row.names = FALSE, col.names = FALSE, sep = " ", na = "0")
 ...
 nb.map <- poly2nb(map)
  ...
  #Scaling R matrix
  scaledR = inla.scale.model(R, constr = list(A = matrix(1, 1, 52), e=0))
  save(R, file = "R.RData")
  head()
  # triangular form
  Qlow <- tril(scaledR)
  trio <- summary(Qlow) # export that - neighbours_scaled.txt file -> blupf90
  summary(trio$i)
  write.table(trio, "neighbours_scaled.txt",
              quote = FALSE, row.names = FALSE, col.names = FALSE, sep = " ", na = "0")
  
 
  ### Check summary of region
  summary (data_clean_test_3$region)
  length(unique(data_clean_test_3$region)) ###46 different regions out of 52 reprensented in our data. Use the crossref of region in the renf90.table in blupf90 to match the each region with their renumbered id.
  sum(is.na(data_clean_test_3$region))
 
   
   ## Colour the map with region estimates from besag1
   region_estimates<- read.csv(file = "region_estimates.csv", stringsAsFactors = FALSE)
   colnames(region_estimates)[1] <- "OBJECTID"
   (.packages())
   ## Reading map South Africa for ploting as sf .
  mapSA <- st_read("/Users/ihouaga/Documents/Project1/MDB_District_Municipal_Boundary_2011.shp", quiet = TRUE)          
  class(mapSA)
   map_estimates<- merge(mapSA,region_estimates,duplicateGeoms = T)
   head(map_estimates)
   save(map_estimates, file = "map_estimates.RData")
  
    # color the map based on region estimates
   ggplot() + geom_sf(data = map_estimates) ## Ploting empty map

ggplot(data = map_estimates, aes(fill = E_GRC1)) + geom_sf() 
ggplot(data = map_estimates, aes(fill = E_GRC2)) + geom_sf()  

#show region number
ggplot(data = map_estimates, aes(fill = E_GRC1)) + geom_sf() +
  scale_fill_gradient(low = "blue", high = "red") + 
  geom_sf_text(data = map_estimates, aes(label = OBJECTID), colour = "black") + labs(fill='Region estimates in GRC1 model') #plot1
  
#No region number

ggplot(data = map_estimates, aes(fill = E_GRC1)) + geom_sf() +
  scale_fill_gradient(low = "blue", high = "red") + 
   labs(fill='Region estimates') #plot1

#show region number GRC2

# ggsave("Plot_region_ERC1E",plot1)
#show region number GRC2
ggplot(data = map_estimates, aes(fill = E_GRC2)) + geom_sf() +
  scale_fill_gradient(low = "blue", high = "red") +
  geom_sf_label(aes(label = OBJECTID)) #plot2

##No region number GRC2
ggplot(data = map_estimates, aes(fill = E_GRC2)) + geom_sf() +
  scale_fill_gradient(low = "blue", high = "red") + 
  labs(fill='Region estimates') #plot1
...
# Plot EBVs_by region
#================================================================================================
EBVs_region<- read.csv(file = "EBVs_region.csv", stringsAsFactors = FALSE)
colnames(EBVs_region)[1] <- "OBJECTID"
map_EBVs_region<- merge(mapSA, EBVs_region,duplicateGeoms = T)
head(map_EBVs_region)
save(map_EBVs_region, file = "map_EBVs_region.RData")
# Ploting sd_EBV_GRC1 showing region's ID
ggplot(data = map_EBVs_region, aes(fill = sd_EBV_GRC1)) + geom_sf() +
  scale_fill_gradient(low = "green", high = "red") +
  geom_sf_label(aes(label = OBJECTID))

# Ploting sd_EBV_GRC2 showing region's ID
ggplot(data = map_EBVs_region, aes(fill = sd_EBV_GRC2)) + geom_sf() +
  scale_fill_gradient(low = "green", high = "red") +
  geom_sf_label(aes(label = OBJECTID))

# Ploting sd_EBV_GRC1 not showing region's ID
ggplot(data = map_EBVs_region, aes(fill = sd_EBV_GRC1)) + geom_sf() +
  scale_fill_gradient(low = "green", high = "red") +
  labs(fill='Standard deviation of EBVs')

# Ploting sd_EBV_GRC2 not showing region's ID
ggplot(data = map_EBVs_region, aes(fill = sd_EBV_GRC2)) + geom_sf() +
  scale_fill_gradient(low = "green", high = "red") +
  labs(fill='Standard deviation of EBVs per region in GRC2 model')
#================================================================================================
# Running Blup with full data without filtering herd for median of records 
#================================================================================================
# Export data_clean_full for blupf90
data_clean_full <- data_clean %>% select(COW_ID,SIRE_ID,DAM_ID,code_HERD_ID,HYS, AGE,PARITY,POSTCODE, Latitude, Longitude, region, MILK_YLD_305D,HY) 
data_clean_full$mean <- rep(1, nrow(data_clean_full))
summary(data_clean_full$MILK_YLD_305D)
write.table(data_clean_full, "data_clean_full.txt",
            quote = FALSE, row.names = FALSE, col.names = FALSE, sep = " ", na = "0")
...
# Plot PEV_by region
#================================================================================================
PEV_region<- read.csv(file = "PEV_region.csv", stringsAsFactors = FALSE)
colnames(PEV_region)[1] <- "OBJECTID"
map_PEV_region<- merge(mapSA, PEV_region,duplicateGeoms = T)
head(map_PEV_region)
save(map_PEV_region, file = "map_PEV_region.RData")

# Ploting mean_PEV_GRC1
ggplot(data = map_PEV_region, aes(fill = mean_PEV_GRC1)) + geom_sf() +
  scale_fill_gradient(low = "green", high = "red") +
  geom_sf_label(aes(label = OBJECTID))
...
ggplot(data = map_PEV_region, aes(fill = mean_PEV_GRC1)) + geom_sf() +
  scale_fill_gradient(low = "green", high = "red") +
  labs(fill='Mean prediction error variance')

# Ploting mean_PEV_GRC2
ggplot(data = map_PEV_region, aes(fill = mean_PEV_GRC2)) + geom_sf() +
  scale_fill_gradient(low = "green", high = "red") +
  geom_sf_label(aes(label = OBJECTID))

#================================================================================================
# Summary of data_clean_final
#================================================================================================
summary(data_clean_final)
data_clean_final$POSTCODE<- as.factor(data_clean_final$POSTCODE)
data_clean_final_H_Y_S<-subset(data_clean_records_2, nrecordsherd >=24) %>%
  separate(HYS, c("H", "Y", "S"))

HerdPostcode_clean<- data_clean_final %>% # Rename later
  select(code_HERD_ID,POSTCODE)

summary(HerdPostcode_clean)
Distinctherdpostcode_clean<- HerdPostcode_clean %>% distinct()
summary(Distinctherdpostcode_clean$POSTCODE)

###Removed herd Postcode 
HerdPostcode_clean2<- data_clean_final %>%
  select(COW_ID, code_HERD_ID,POSTCODE,PARITY)
summary(HerdPostcode_clean2)
Distinctherdpostcode_clean2<- HerdPostcode_clean2 %>% distinct()
summary(Distinctherdpostcode_clean2$POSTCODE)
length(unique(HerdPostcode_clean2$POSTCODE))## 349 unique postcode
### New Number of records
length(HerdPostcode_clean2$COW_ID) #248597
### New Number cows
length(unique(HerdPostcode_clean2$COW_ID))#98632
### New Number herds
length(unique(HerdPostcode_clean2$code_HERD_ID)) # 1145
summary(HerdPostcode_clean2$PARITY)
### Postcode statistics

# Number of records per cow per Postcode # HerdPostcode_clean2
ncowpostcode10_clean<- HerdPostcode_clean2 %>%
  group_by(POSTCODE, COW_ID)%>%
  summarise(ncowpostcode10_clean= n())
summary(ncowpostcode10_clean$ncowpostcode10_clean)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#1.000   1.000   2.000   2.456   3.000   9.000 

# Number of  cow per postcode # data_clean_final
ncowpostcode10_clean2<- ncowpostcode10_clean %>%
  select(POSTCODE) %>%
  summarise(ncowbypostcode10=n()) 
summary(ncowpostcode10_clean2$ncowbypostcode10) 
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#10      44     117     290     316    4276 

# Number of  herds per postcode # dataHSTnew10
nherdpostcode10_clean<- HerdPostcode_clean2 %>%
  group_by(POSTCODE, code_HERD_ID)%>%
  summarise(nherd_bypostcode10= n())
summary(nherdpostcode10_clean$nherd_bypostcode10) 
nherdpostcode10_clean2<- nherdpostcode10_clean %>%
  select(POSTCODE) %>%
  summarise(nherdinpostcode10=n())
summary(nherdpostcode10_clean2$nherdinpostcode10)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#1.000   1.000   2.000   3.281   4.000  30.000 

# Number of records per cow per herd
ncowherd11_clean<- HerdPostcode_clean2 %>%
  group_by(code_HERD_ID, COW_ID)%>%
  summarise(ncow_byherd= n())
summary(ncowherd11_clean$ncow_byherd)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#1.000   1.000   2.000   2.386   3.000   9.000  

# Number of  cow per herd
ncowherd11_clean2<- ncowherd11_clean %>%
  select(code_HERD_ID) %>%
  summarise(ncowherd=n())
summary(ncowherd11_clean2$ncowherd)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#8.00   25.00   45.00   90.98   98.00 2230.00 
colnames(data_clean_final_allvariables)
data_clean_final_H_Y_S$Y <- as.factor(data_clean_final_H_Y_S$Y)
summary(data_clean_final_H_Y_S$Y) # Calving Year between 1957 and 2014.
